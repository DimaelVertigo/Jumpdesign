...
class ffMetaBoxOnePageView extends ffMetaBoxView {
	...
	protected function _render( $post ) {
		...
		$s = $fwc->getOptionsFactory()->createOptionsHolder('ffComponent_Theme_DefaultOptions')->getOptions();
		...
		echo '<div class="ff-revision-list">';
			...
			echo '<ul style="margin-top:4px">';
				...
				foreach( $revisionManager->getListOfRevisionsForCurrentPost() as $revisionNumber => $oneRevision ) {
					...
					echo '<li>';
						echo 'Revision ' . ff_wp_kses( $oneRevision['number'] ) . ' (' . ff_wp_kses( $oneRevision['human_time'] ).') ';


						if( $revisionNumber == $currentRevisionNumber ) {
							echo 'Current';
						} else {
							echo '<a href="" class="ff-revision-switch" data-revision-number="'.esc_attr( $revisionNumberText ).'">';
								echo 'Rollback';
							echo '</a>';
						}
					echo '</li>';
