...
if( !function_exists('ff_print_before_section') ) {
	...
	{
		$sectionName = $query->getVariationType();

		if( $query->queryExists('section-settings' ) ) {
			$query = $query->get('section-settings');
		}
		/**********************************************************************************************************************/
		...
		echo '<div class="ff-section-fullwidth ff-section-fullwidth-'.esc_attr($sectionName).esc_attr($colorClass).esc_attr($fullscreenClass).'" '.($styleString).' '.esc_attr($id).'>';
		...
		echo '<div class="ff-section-boxed ff-section-boxed-'.esc_attr($sectionName).esc_attr($fullscreenClass).'" '.($styleString).'>';
...
if( !function_exists('ff_get_section_preview_image_url') ) {
	...
	{
		return get_template_directory_uri() . '/templates/onePage/sections/' .  $name . '/' .  $name . '.png';
	}
}
