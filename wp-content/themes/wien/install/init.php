...
/**********************************************************************************************************************/
    private function _installFrameworkForFirstTime( $zipFilePath ) {
        ...
        if( $unzipFileResult instanceof WP_Error ) {
            $notificationText = __('Fresh Framework requires wp-content/plugins folder, to be writable, in order to install itself, ', 'wien');
            $notificationText .= __('this could be achieved by setting 775 or 777 permissions to wp-content/plugins folder, ', 'wien');
            $notificationText .= __('please install Fresh Framework plugin, otherwise our theme will not work<br><br>', 'wien');
            $notificationText .= __('Plugin could be found here: ', 'wien').get_template_directory().'/install/fresh-framework.zip';
    ...
    private function _getFileSystem() {
        ...
        if( $method != 'direct' ) {
            $notificationText = __('Fresh Framework requires File System Direct to install itself. If you cant achieve this, ','wien');
            $notificationText .= _('please install Fresh Framework plugin, otherwise our theme will not work<br><br>','wien');
            $notificationText .= __('Plugin could be found here: ','wien').get_template_directory().'/install/fresh-framework.zip';
