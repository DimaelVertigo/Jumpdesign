<?php
	...
	$postMetaOptions = ffContainer()->getDataStorageFactory()->createDataStorageWPOptions();
	$relatedProjectsArray = array();
			...
			foreach( $posts as $onePost ) {
				...
				$data = $postMetaOptions->getOption( $onePost->getID(), 'portfolio_category_options');
		...
		?>
			...
			<article class="project">
				...
				<header class="project-header">
					<h4 class="project-title"><a href="<?php echo get_the_permalink( $onePost->getID() ); ?>"><?php echo get_the_title( $onePost->getID() ); ?></a></h4>
	...
	<div class="projects row">
		...
		<?php
			...
			echo '<div class="'.esc_attr($colClass).'">';
				echo implode( '</div><div class="'.esc_attr($colClass).'">', $relatedProjectsArray );
			echo '</div>';
		?>
