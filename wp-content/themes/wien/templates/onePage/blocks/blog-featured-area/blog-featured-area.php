...
if( ! empty( $featured_primary ) ){
	...
	if( $imageUrlResized != null ) {
		...
		<div class="blog-article-thumbnail">
			...
			<?php
				if( $query->get('lightbox') == 1 ||  $query->get('lightbox') == 2 ) {

					$class = '';
					if( $query->get('lightbox') == 1 ){
						$class = 'fancybox-blog-gallery';
					} else if ( $query->get('lightbox') == 2 ) {
						$class = '';
						$imageUrlNonresized = get_permalink();
					}
			?>
			<div class="blog-article-hover">
				<a class="<?php echo esc_attr( $class ); ?> zoom-action" href="<?php echo esc_url( $imageUrlNonresized ); ?>">
					<img src="<?php echo get_template_directory_uri(); ?>/images/backgrounds/glasses.png" alt="">
				</a>
			</div>
			<?php } ?>
		</div>
